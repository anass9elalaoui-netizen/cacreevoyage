...
async function seed() {
  ...
  console.log('🌍 Seeding 13 destinations...')
  ...
  for (const dest of destinationsSeed) {
    ...
    if (existing.docs.length > 0) {
      ...
      console.log(`  ✅ Updated: ${dest.title} → /destinations/${slug}`)
    } else {
      // Create new (headerMedia is now optional)
      ...
      console.log(`  ✅ Created: ${dest.title} → /destinations/${slug}`)
    }
  }
