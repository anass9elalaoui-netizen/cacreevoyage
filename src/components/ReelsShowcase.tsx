...
function ReelCard({ testimonial }: { testimonial: Testimonial }) {
  ...
  return (
    <motion.div
      ...
      className="shrink-0 snap-center relative aspect-[9/16] w-[280px] md:w-[320px] rounded-[2.5rem] overflow-hidden border border-white/10 shadow-2xl cursor-pointer bg-brand-dark/50 hover:shadow-[0_20px_50px_rgba(0,0,0,0.5)] transition-shadow duration-300"
    >
      <video
        ref={videoRef}
        src={videoUrl}
      ...
      <button 
        onClick={toggleMute}
        className="absolute top-6 right-6 z-20 w-10 h-10 rounded-full bg-black/40 backdrop-blur-md border border-white/20 flex items-center justify-center hover:bg-black/60 transition-colors"
      >
        {isMuted ? (
      ...
      <div className="absolute bottom-6 left-6 right-6 z-20">
        <div className="p-4 rounded-2xl bg-white/10 backdrop-blur-xl border border-white/20 shadow-lg flex flex-col">
          <span className="text-white font-medium text-lg mb-1 drop-shadow-md">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-white/80 text-sm font-light truncate drop-shadow-md">
