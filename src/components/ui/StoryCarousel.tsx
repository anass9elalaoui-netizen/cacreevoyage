...
function StoryCard({
  ...
  return (
    <motion.div
      className="relative flex-shrink-0 overflow-hidden rounded-[32px]"
      style={{
...
export default function StoryCarousel({ itinerary }: { itinerary: StoryDay[] }) {
  ...
  const getDragConstraints = useCallback(() => {
    ...
    const maxDrag = -(totalWidth - viewportW + 40) // 40px padding
    return { left: Math.min(maxDrag, 0), right: 0 }
  ...
  const handleDragEnd = useCallback(
    (_: any, info: PanInfo) => {
      ...
      const viewportW = window.innerWidth
      const targetX = -(newIndex * step) + (viewportW / 2 - cardW / 2)
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const cardW = getCardWidth()
      const viewportW = window.innerWidth
      const targetX = viewportW / 2 - cardW / 2
  ...
  const goTo = useCallback(
    (index: number) => {
      ...
      const step = cardW + gap
      const viewportW = window.innerWidth
      const targetX = -(clamped * step) + (viewportW / 2 - cardW / 2)
    ...
    <div className="relative w-full overflow-hidden">
      ...
      <div className="hidden md:flex items-center justify-end gap-3 mb-6 px-4 md:px-12 lg:px-20">
        <button
          ...
          className="w-12 h-12 rounded-full border border-white/20 bg-white/5 backdrop-blur-md flex items-center justify-center text-white hover:bg-white/10 transition-all hover:scale-105 active:scale-95 disabled:opacity-30 disabled:hover:scale-100"
